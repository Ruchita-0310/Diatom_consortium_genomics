...
phaeo_hits_file = base / "08_phaeodactylum/18_diatom_nuclear_v1_genes_with_phaeodactylum_hits.tsv"
...
required_cols = ["protein_id", "transdecoder_orf_id", "contig_id", "gene_length_bp", "Average_TPM"]
...
phaeo = pd.read_csv(phaeo_hits_file, sep="\t", names=phaeo_cols, dtype=str, low_memory=False)
phaeo_gene_roots = set(phaeo["diatom_gene_id"].dropna().astype(str).str.strip())
...
front_cols = [
    "gene_id", "transdecoder_orf_id", "compartment", "gene_length_bp",
    "recommended_annotation", "recommended_annotation_source", "recommended_annotation_confidence",
    "Average_TPM", "in_Phaeodactylum_tricornutum",
    "has_any_annotation", "has_swissprot_hit", "has_bacillariophyta_hit",
    "has_interproscan_hit", "has_antifam_flag",
    "antifam_accession", "antifam_description", "antifam_flag",
    "swissprot_accession", "swissprot_entry", "swissprot_protein_name",
    "swissprot_organism", "swissprot_gene_name", "swissprot_evalue",
    "swissprot_bitscore", "swissprot_pident", "swissprot_qcov_percent",
    "swissprot_scov_percent", "swissprot_hit_confidence",
    "bacillariophyta_accession", "bacillariophyta_entry", "bacillariophyta_protein_name",
    "bacillariophyta_organism", "bacillariophyta_gene_name", "bacillariophyta_evalue",
    "bacillariophyta_bitscore", "bacillariophyta_pident", "bacillariophyta_qcov_percent",
    "bacillariophyta_scov_percent", "bacillariophyta_hit_confidence",
    "interproscan_analyses", "signature_accessions", "signature_descriptions",
    "interpro_accessions", "interpro_descriptions", "go_terms", "pathway_annotations",
    "contig_id", "gene_model_start", "gene_model_end", "strand", "cds_length_bp", "protein_length_aa",
]
...
pt_detail_cols = [
    ...
    "diatom_gene_score", "diatom_gene_strand", "hit_contig", "hit_start", "hit_end",
    "blast_hit_id", "bitscore", "blast_strand", "pt_contig", "pt_start", "pt_end",
    "pident", "aln_len", "evalue", "pt_gene_id", "pt_gene_name", "pt_locus_tag",
]
...
if "recommended_annotation" in final.columns:
    ...
    final.loc[final["recommended_annotation"].astype(str).str.strip() == "", "recommended_annotation"] = "unannotated"

final["Average_TPM"] = pd.to_numeric(final["Average_TPM"], errors="coerce")
final.to_csv(out_file, sep="\t", index=False)

sorted_final = final.sort_values(by="Average_TPM", ascending=False, na_position="last")
...
top_cols = ["gene_id", "transdecoder_orf_id", "compartment", "recommended_annotation", "Average_TPM", "in_Phaeodactylum_tricornutum"]
